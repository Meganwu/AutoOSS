...
class DissociateEnv:
        ...
        def step(self, 
                ...
                # if self.detect_mol_cnn(self.diss_after_img):
                ...
                # if self.detect_product_cnn(self.diss_after_img)=='origin_mol':
                #         if self.detect_product_cnn_2(self.diss_after_img)=='origin_mol':                 
                #                 done_diss=False
                #                 find_mol=True
                #         else:
                ...
                # elif self.detect_product_cnn(self.diss_after_img)=='diss_mol':
                #         if self.detect_product_cnn_2(self.diss_after_img)=='diss_mol':
                #                 done_diss=True
                #                 find_mol=False
                #         else:
                #                 done_diss=False
        ...
        # def compute_reward_1(self)->float:             # default reward
        #         """
        ...
        #         elif self.old_detect_diss(topography) and product!='messy_mol' and product2=='origin_mol':
        #                 reward = -0.5

        #         elif self.diff_topography<2.0 and product!='messy_mol' and product2=='origin_mol':
        #                 reward = np.min([self.diff_topography, 2.5])/8.0

        #         elif self.old_detect_diss(topography) and product=='messy_mol':
        #                 reward = -2
        ...
        # def compute_reward(self)->float:                       # rough reward
        #         """
        #         Calculate the reward after dissociation
        ...
        # def compute_reward(self, step_i, f_succ=0.9, f_rotation=0.85, f_messy=0.92, bias_messy=0.3)->float:
        #         """
        #         Calculate the reward after dissociation
        ...
        #         product2=self.detect_product_cnn_2(self.diss_after_img)
        #         if product!='messy_mol' and product2=='diss_mol':
        #                 reward = 1.0*f_succ**step_i


        #         elif product!='messy_mol' and product2=='origin_mol':
        #                 reward = np.min([self.diff_topography, 2.5])/20.0*f_rotation**step_i

        #         else:
        #                 reward = -0.7/(f_messy**step_i)+bias_messy
        
        #         return reward
        

        def compute_reward(self, step_i, f_succ=0.9, f_rotation=1.05, f_messy=0.95, bias_messy=0.3)->float:        # default reward
                """
                Calculate the reward after dissociation
                ...
                if product!='messy_mol' and product2=='diss_mol':
                        ...
                        reward = np.min([self.diff_topography, 2.5])/2.5/20.0-0.6*f_rotation**step_i + 0.5
        ...
        # def compute_reward(self, step_i, f_succ=0.9, f_rotation=1.05, f_messy=0.95, bias_messy=0.3)->float:        # rough reward
        #         """
        #         Calculate the reward after dissociation
        ...
        #         topography=self.diss_topography
        #         self.old_detect_diss(topography)
        #         product=self.detect_mol_cnn(self.diss_after_img)
        #         product2=self.detect_product_cnn_2(self.diss_after_img)
        #         if product and product2=='diss_mol':
        #                 reward = 1.0*f_succ**step_i


        #         elif product and product2=='origin_mol':
        #                 reward = np.min([self.diff_topography, 2.5])/20.0*f_rotation**step_i
        ...
        def random_action_from_table_fix_pos(self, table_name='tip_pos_data_action_succ_12_5_15_15_209.csv'):
                """
                Generate random actions for dissociation
                ...
                # index=np.random.choice(action_table.index, 1)[0]
                action_table=action_table.loc[[206, 202]].reset_index()
                index=np.random.choice(action_table.index, 1)[0]
                mvoltage_frac=action_table.loc[index].diss_mvoltage/self.max_mvoltage
                pcurrent_frac=action_table.loc[index].diss_pcurrent/self.max_pcurrent
                pos_noise=(np.random.random(1)*np.pi)[0]
                tip_x_frac=(self.tip_pos_expert_nm[0]+0.1*np.sin(pos_noise))/self.diss_radius_nm
                tip_y_frac=(self.tip_pos_expert_nm[1]+0.1*np.cos(pos_noise))/self.diss_radius_nm
                action = [tip_x_frac, tip_y_frac, mvoltage_frac, pcurrent_frac]

                return action
        
        def random_action_from_table_fix_pos_random_pulse(self, table_name='tip_pos_data_action_succ_5_31_22_48_3.csv'):
                """
                Generate random actions for dissociation
        ...
        def rotate_tip_pos(self, ref_tip_pos, ref_ell_angle, new_ell_angle):
                """
                Rotate the tip position
                ...
                angle: float
                        the angle to rotate the tip

                Returns
                -------
                tip_pos: array_like
                        the rotated tip position
                """
                ref_x, ref_y = tip_pos
                ref_len=np.sqrt(ref_x**2+ref_y**2)
                ref_angle=np.arctan2(ref_y, ref_x)
                x2=ref_len*math.sin((ref_angle+(ref_ell_angle-new_ell_angle))/180*np.pi)
                y2=ref_len*math.cos((ref_angle+(ref_ell_angle-new_ell_angle))/180*np.pi)
        ...
        # def action_to_diss_input(self, action):         # traditional 4D
        #         """
        ...
        # def action_to_diss_input(self, action):             #  4D fix pos
        #         """
        #         Convert the action to the input for the dissociation
        ...
        #                 return 'origin_mol'
        #         elif np.argmax(pred_label.detach().numpy())==1 or np.argmax(pred_label.detach().numpy())==2:
                ...
                if new_scan_region:
                        ...
                        while not found_mol:
                                ...
                                for i in range(candidate_mols_num):
                                        ...
                                        if  img_cnn_detect_mol and self.detect_product_cnn(img_small)!='messy_mol' and self.detect_product_cnn_2(img_small)=='origin_mol':
        ...
        def get_next_region(self, tip_x, tip_y, scan_ref_x_center=None, scan_ref_y_center=None, forbid_radius=20, check_simi_forbid_radius=25, move_upper_limit=400, approach_limit=[-300, 300, -300, 300], spiralreg=1.0, mn=100, detect_similarity=True, check_similarity=None):
                ...
                #         if tip_x-approach_limit[0]<approach_limit[1]-tip_x:
                #                 tip_x=tip_x+50*np.random.random()
                #         else:
                #                 tip_x=tip_x-50*np.random.random()
                #         if tip_y-approach_limit[2]<approach_limit[3]-tip_y:
                #                 tip_y=tip_y+50
                #         else:
                #                 tip_y=tip_y-50
                #        return tip_x, tip_y
                        ...
                        tip_x=approach_limit[0]+100+550*np.random.random()
                        tip_y=approach_limit[2]+100+550*np.random.random()
